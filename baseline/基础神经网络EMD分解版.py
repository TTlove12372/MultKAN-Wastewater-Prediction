...
def select_features_with_rf(X, y, top_n=10, log_file=None):
    ...
    log_message(str(top_features), log_file)

    # 选择前N个特征
    selected_features = feature_importances.head(top_n)['feature'].tolist()
    log_message(f"选中了{len(selected_features)}个特征:", log_file)
...
def apply_emd_decomposition(X, data_index, log_file=None):
    ...
    if nan_columns:
        log_message(f"以下列仍有NaN值，已进行填充: {nan_columns}", log_file)
...
def prepare_multkan_data(X, y, log_file=None):
    ...
    # 为MultKAN模型缩放目标变量
    y_array = y.reshape(-1, 1) if isinstance(y, np.ndarray) else y.values.reshape(-1, 1)
