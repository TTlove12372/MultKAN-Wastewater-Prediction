...
def main():
    ...
    try:
        ...
        # 将特征直接转换为NumPy数组，不再进行特征工程
        X_processed = X.values
        y_values = y.values

        # 检测并移除异常值 - 改为不处理异常值以保持数据一致性
        X_processed, y_values = detect_and_remove_outliers(X_processed, y_values, method='none')
        ...
        if gpu_available:
            ...
            xgb_fixed_params = {
                'objective': 'reg:squarederror',
                'tree_method': 'hist'  # 使用CPU直方图方法
            }

        # 创建基础模型
        xgb_model = xgb.XGBRegressor(**xgb_fixed_params)
        ...
        xgb_random_search = RandomizedSearchCV(
            ...
            verbose=1,
            random_state=42,
            n_jobs=1,  # 使用单线程避免并行处理问题
            error_score='raise'  # 出错时抛出详细异常
        )
        ...
        # 记录最佳参数
        log_message(f"XGBoost 最佳参数: {xgb_random_search.best_params_}", log_file)
        log_message(f"XGBoost 训练时间: {xgb_end - xgb_start:.2f} 秒", log_file)

        # 保存模型
        xgb_best_model.save_model(os.path.join(output_dir, "models", "xgboost_best_model.json"))

        # 评估模型
        xgb_results, xgb_train_pred, xgb_test_pred = evaluate_model(
            xgb_best_model, X_train_scaled, y_train, X_test_scaled, y_test, "XGBoost", log_file)
        ...
        # 创建基础模型
        lgb_model = lgb.LGBMRegressor(**lgb_fixed_params)
        ...
        # 记录最佳参数
        log_message(f"LightGBM 最佳参数: {lgb_random_search.best_params_}", log_file)
        log_message(f"LightGBM 训练时间: {lgb_end - lgb_start:.2f} 秒", log_file)

        # 保存模型
        joblib.dump(lgb_best_model, os.path.join(output_dir, "models", "lightgbm_best_model.pkl"))

        # 评估模型
        lgb_results, lgb_train_pred, lgb_test_pred = evaluate_model(
            lgb_best_model, X_train_scaled, y_train, X_test_scaled, y_test, "LightGBM", log_file)
