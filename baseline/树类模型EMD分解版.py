...
def detect_and_remove_outliers(X, y, method='IQR', threshold=1.5):
    ...
    if method == 'IQR':
        ...
        outliers_removed = len(y) - len(y_cleaned)
        log_message(f"已移除 {outliers_removed} 个异常值 ({outliers_removed / len(y) * 100:.2f}%)")
        log_message(f"清洗后数据形状: {X_cleaned.shape}, {y_cleaned.shape}")
...
def select_features_with_rf(X, y, top_n=10, log_file=None):
    ...
    log_message(f"选中了{len(selected_features)}个特征用于模态分解:", log_file)
...
def apply_emd_decomposition(X, data_index, log_file=None):
    """对选中的特征应用EMD模态分解"""
    log_message("\n开始对选中的输入变量进行模态分解...", log_file)

    # 创建EMD实例
    emd = EMD()

    # 创建一个DataFrame来存储所有分解后的特征
    X_decomposed = pd.DataFrame(index=data_index)

    # 对选中的输入变量进行模态分解
    for col in X.columns:
        # 获取信号并预处理
        signal = handle_nan(X[col].values)
        ...
        try:
            ...
            for i, imf in enumerate(imfs):
                ...
                if len(imf) == len(signal):
                    imf = handle_nan(imf)  # 确保没有NaN
                    X_decomposed[f"{col}_imf{i}"] = imf
                else:
                    log_message(f"警告: {col}_imf{i}长度不匹配，调整长度", log_file)
                    # 调整IMF长度以匹配原始长度
                    if len(imf) > len(signal):
                        imf = imf[:len(signal)]
                    else:
                        imf = np.pad(imf, (0, len(signal) - len(imf)), 'constant')
                    imf = handle_nan(imf)
                    X_decomposed[f"{col}_imf{i}"] = imf
        except Exception as e:
            log_message(f"对{col}进行EMD分解出错: {e}", log_file)
            # 如果分解失败，使用原始信号
            X_decomposed[col] = signal

    # 最终检查并处理任何可能剩余的NaN值
    log_message("检查并处理任何残余的NaN值...", log_file)
    nan_columns = []
    for col in X_decomposed.columns:
        if X_decomposed[col].isna().any():
            nan_columns.append(col)
            X_decomposed[col] = X_decomposed[col].fillna(
                X_decomposed[col].mean() if not pd.isna(X_decomposed[col].mean()) else 0)

    if nan_columns:
        log_message(f"以下列仍有NaN值，已进行填充: {', '.join(nan_columns)}", log_file)

    log_message(f"原始特征数: {X.shape[1]}", log_file)
    log_message(f"分解后特征数: {X_decomposed.shape[1]}", log_file)

    return X_decomposed


def prepare_multkan_data(X, y, log_file=None):
    """为MultKAN模型准备和缩放数据"""
    log_message("\n为MultKAN模型准备数据...", log_file)
    ...
    # 为MultKAN模型缩放目标变量
    y_array = y.values.reshape(-1, 1) if isinstance(y, pd.Series) else y.reshape(-1, 1)
    y_scaler = StandardScaler()
    y_normalized = y_scaler.fit_transform(y_array)

    log_message(f"MultKAN数据准备完成 - X形状: {X_normalized.shape}, y形状: {y_normalized.shape}", log_file)
...
def main():
    ...
    try:
        # 替换为实际的文件路径
        file_path = r"C:\Users\肖亚宁\PycharmProjects\pythonProjecttensor\最终\平定6指标_cleaned\平定6指标_cleaned\cleaned_平定中伟—COD.xlsx"
        ...
        # 3. 对选中的特征应用EMD分解
        X_decomposed = apply_emd_decomposition(X_selected, data.index, log_file=log_file)

        # 4. 为MultKAN准备数据（可选，保存备用）
        X_normalized, y_normalized, X_scaler, y_scaler = prepare_multkan_data(X_decomposed, y, log_file=log_file)

        # 将特征工程后的数据准备为NumPy数组用于训练模型
        X_processed = X_decomposed.values
