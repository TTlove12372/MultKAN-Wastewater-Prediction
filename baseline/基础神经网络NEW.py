...
def main():
    ...
    try:
        ...
        try:
            ...
            # ==================== 使用原始特征 ====================
            # 注意：这里已移除滞后特征加入、随机森林和EMD分解

            # 将X转换为numpy数组，直接使用原始特征
            X_for_ml = X.values
            y_filtered = y.values
            ...
            bpnn_tf_results, bpnn_tf_history, bpnn_tf_train_pred, bpnn_tf_test_pred = train_evaluate_tf_model(
                bpnn_tf_model, X_train_scaled, y_train, X_test_scaled, y_test,
                "BPNN_TensorFlow", epochs=50, batch_size=32, patience=30, log_file=log_file
            )
            all_results.append(bpnn_tf_results)
