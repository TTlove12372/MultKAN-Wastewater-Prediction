...
print(f"选中了{len(selected_features)}个特征:")
...
print(f"选中特征数: {X_selected.shape[1]}")

# 准备MultKAN模型的数据
X_scaler = MinMaxScaler(feature_range=(-1, 1))
X_normalized = X_scaler.fit_transform(X_selected)  # 直接使用选中的特征
...
# 配置MultKAN模型
input_dim = X_selected.shape[1]  # 使用选中特征的维度
hidden_layers = [72, 64]
output_dim = 1
grid_size = 9
k_value = 3
...
# 保存模型参数和指标到一个Excel文件
model_params_file = 'MultKAN模型参数_无EMD.xlsx'
...
results_file = 'MultKAN预测结果_无EMD.xlsx'
...
plt.title('MultKAN模型预测结果（无EMD）')
plt.grid(True)
plt.savefig('prediction_results_no_emd.png')
plt.show()
...
if 'date' in data.columns or '日期' in data.columns:
    ...
    plt.title('COD时间序列预测（无EMD）')
    plt.legend()
    plt.grid(True)
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('time_series_no_emd.png')
    plt.show()
else:
    # 如果没有日期列，使用样本索引
    plt.figure(figsize=(14, 7))
