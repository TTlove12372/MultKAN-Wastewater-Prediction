...
from sklearn.ensemble import RandomForestRegressor  # Added for feature selection
from kan.MultKAN import MultKAN
from PyEMD import EMD  # 引入EMD模块用于模态分解
...
def handle_nan(signal):
    ...
    if np.isnan(signal).any():
        ...
        if len(valid_indices) > 0:
            # 使用线性插值填充NaN
            signal[mask] = np.interp(indices[mask], valid_indices, signal[~mask])
        else:
            # 如果全是NaN，用0填充
            signal[mask] = 0
    return signal
...
# 打印特征重要性
print("特征重要性排名:")
print(feature_importances.head(10))  # 显示前10个最重要的特征

# 选择前N个特征
top_n_features = 3 # 选择前N个最重要的特征
selected_features = feature_importances.head(top_n_features)['feature'].tolist()
print(f"选中了{len(selected_features)}个特征用于模态分解:")
...
X_decomposed = pd.DataFrame(index=data.index)

# 对选中的输入变量进行模态分解
print("开始对选中的输入变量进行模态分解...")
for col in X_selected.columns:
    # 获取信号并预处理
    signal = handle_nan(X_selected[col].values)
    ...
    try:
        ...
        for i, imf in enumerate(imfs):
            ...
            if len(imf) == len(signal):
                ...
                print(f"警告: {col}_imf{i}长度不匹配，调整长度")
                # 调整IMF长度以匹配原始长度
                if len(imf) > len(signal):
                    imf = imf[:len(signal)]
                else:
        ...
        print(f"对{col}进行EMD分解出错: {e}")
        # 如果分解失败，使用原始信号
        X_decomposed[col] = signal

# 最终检查并处理任何可能剩余的NaN值
print("检查并处理任何残余的NaN值...")
for col in X_decomposed.columns:
    if X_decomposed[col].isna().any():
        print(f"列 {col} 仍有NaN值，进行填充")
        X_decomposed[col] = X_decomposed[col].fillna(X_decomposed[col].mean())

print(f"原始特征数: {X.shape[1]}")
print(f"分解后特征数: {X_decomposed.shape[1]}")

# 准备MultKAN模型的数据
X_scaler = MinMaxScaler(feature_range=(-1, 1))
X_normalized = X_scaler.fit_transform(X_decomposed)
...
# 配置MultKAN模型
input_dim = X_decomposed.shape[1]
hidden_layers = [72,64]
output_dim = 1
grid_size = 9
k_value = 3
...
# 保存模型参数和指标到一个Excel文件
model_params_file = 'MultKAN模型参数.xlsx'
